...
from data import get_mnist_data, num_classes, input_shape, get_fashionmnist_data, get_cifar_data
from read_activations import get_activations, display_activations

if __name__ == '__main__':

    checkpoints = glob('checkpoints_cifar/*.h5')
    ...
    if len(checkpoints) > 0:
        ...
        try:
            shutil.rmtree('checkpoints_cifar')
        except:
            pass
        os.mkdir('checkpoints_cifar')

        checkpoint = ModelCheckpoint(monitor='val_acc',
                                     filepath='checkpoints_cifar/model_{epoch:02d}_{val_acc:.3f}.h5',
                                     save_best_only=True)
